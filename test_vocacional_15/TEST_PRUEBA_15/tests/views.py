...
def test_view(request):
    if request.method == 'POST':
        ...
        if conteo:
            ...
            LogAccion.objects.create(usuario=request.user, accion=f"Realizó el test vocacional. Resultado: {resultado}")
        else:
...
def login_view(request):
    if request.method == "POST":
        ...
        if user is not None:
            login(request, user)
            LogAccion.objects.create(usuario=user, accion="Inició sesión")
...
def logout_view(request):
    if request.user.is_authenticated:
        LogAccion.objects.create(usuario=request.user, accion="Cerró sesión")
    logout(request)
...
def ver_logs(request):
    logs = LogAccion.objects.all().order_by('-fecha')
    return render(request, 'tests/ver_logs.html', {'logs': logs})
